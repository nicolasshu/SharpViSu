...
elseif show == 5 % density
    if mode == 4
        Dens = 1000 * stats(:,1) ./ stats(:,2); % events/�m^2
    else
        I0 = handles.I0{channel};
        s = size(stats, 1);
        Dens = zeros(s,1);
        for i = 1:s
            ...
            area = stats(i).Area;
            Dens(i) = NBeventsi/(area * (p * 0.001)^2); % events/�m^2
        end
    end
    ...
    set(handles.text46, 'String', 'density of events in cluster, 1/�m');
end
end
...
%                 hgexport(h1, FPName, hgexport('factorystyle'), 'Format', 'png');
             ...
             if base ~= 4
                ...
                for i = 1:s
                    ind = stats(i).PixelIdxList;
                    NBeventsi = sum(I0(ind));
                    Dens(i) = NBeventsi/(area(i) * 0.001^2); % events/�m^2
                end
             else
                 Dens = 1000 * stats(:,1) ./ stats(:,2); % events/�m^2
             end
            ...
            elseif channel == 2 %green eventlist
                ...
                xlabel('density of events in cluster, 1/�m')
                ylabel('occurrence')
                savefig(h1,FPName)
%                 hgexport(h1, FPName);
%                 hgexport(h1, FPName, hgexport('factorystyle'), 'Format', 'png');
